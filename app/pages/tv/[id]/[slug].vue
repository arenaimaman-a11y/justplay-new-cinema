    ...
    <div class="container mx-auto px-4 md:px-8 max-w-4xl relative z-10">
        ...
        <div class="flex items-end sm:items-center gap-3 relative self-start md:self-auto">
          ...
          <div class="relative">
            ...
            <button 
              ...
              class="flex items-center gap-2 bg-gray-900/60 hover:bg-gray-900 border border-white/[0.05] hover:border-white/[0.1] text-xs font-bold text-gray-200 px-4 py-2.5 rounded-xl transition-all duration-300 shadow-md active:scale-95"
            >
            ...
            <transition name="dropdown">
              <div v-if="activeDropdown === 'season'" class="absolute right-0 mt-2 w-40 bg-gray-900/95 backdrop-blur-xl border border-white/[0.08] rounded-xl shadow-2xl p-1.5 z-50 space-y-0.5 max-h-60 overflow-y-auto custom-scrollbar">
                <button 
                  v-for="season in validSeasons" 
                  :key="season.id"
                  @click="selectSeason(season.season_number)"
                  :class="[
                    'w-full text-left px-3 py-2 text-xs font-semibold rounded-lg transition-all duration-200 block',
                    selectedSeasonNumber === season.season_number ? 'bg-red-600 text-white font-bold' : 'text-gray-400 hover:bg-white/[0.04] hover:text-white'
                  ]"
                >
          ...
          <div class="relative">
            ...
            <button 
              @click="toggleDropdown('episode')"
              :disabled="!currentSeasonDetails?.episodes"
              class="flex items-center gap-2 bg-gray-900/60 hover:bg-gray-900 border border-white/[0.05] hover:border-white/[0.1] text-xs font-bold text-gray-200 px-4 py-2.5 rounded-xl transition-all duration-300 shadow-md active:scale-95 disabled:opacity-40"
            >
            ...
            <transition name="dropdown">
              <div v-if="activeDropdown === 'episode' && currentSeasonDetails?.episodes" class="absolute right-0 mt-2 w-44 bg-gray-900/95 backdrop-blur-xl border border-white/[0.08] rounded-xl shadow-2xl p-1.5 z-50 space-y-0.5 max-h-60 overflow-y-auto custom-scrollbar">
                <button 
                  v-for="ep in currentSeasonDetails.episodes" 
                  :key="ep.id"
                  @click="selectEpisode(ep.episode_number)"
                  :class="[
                    'w-full text-left px-3 py-2 text-xs font-semibold rounded-lg transition-all duration-200 flex items-center justify-between',
                    currentEpisode === ep.episode_number ? 'bg-red-600 text-white font-bold' : 'text-gray-400 hover:bg-white/[0.04] hover:text-white'
                  ]"
                >
      ...
      <div class="mt-10 border-b border-gray-900/80 pb-8">
        ...
        <div class="flex items-center gap-3 mt-2 text-xs text-gray-500 font-semibold tracking-wide">
          ...
          <span v-if="tvDetails?.vote_average" class="text-yellow-500 flex items-center gap-1 bg-yellow-500/5 px-2 py-0.5 rounded border border-yellow-500/10">
        ...
        <div class="relative mt-4 transition-all duration-500 overflow-hidden" :class="isExpanded ? 'max-h-[1000px]' : 'max-h-24'">
          <p class="text-xs md:text-sm text-gray-400 leading-relaxed max-w-3xl font-medium">
            {{ tvDetails?.overview || 'No synopsis metadata available for this television network entry.' }}
          </p>

          <div class="mt-4 pt-4 border-t border-white/[0.03] space-y-3 text-[11px] md:text-xs text-gray-500 leading-relaxed max-w-3xl">
            <p>
              Looking for a way to watch <strong class="text-gray-400">{{ tvNameClean }} Season {{ currentSeason }} Episode {{ currentEpisode }}</strong> online free? You have landed on the premium destination. On JusPlay, we provide full access to stream your favorite television series, award-winning dramas, and popular shows in ultra-high definition quality without requiring any paid subscriptions. 
            </p>
            <p>
              Our system continuously monitors video streams to offer lightning-fast load times. By utilizing optimized multimedia proxy technologies and high-end alternative player servers like <span class="text-red-500/70">Server 1, Server 2, and Server 3</span>, we guarantee uninterrupted streaming sessions, effectively neutralizing standard video buffering issues, corrupted database playback, or sudden black screens.
            </p>
            <p>
              Subtitles are fully integrated across all major global network integrations, offering multiple language localizations including English CC, Spanish, and Indonesian translations. Bookmark our main domain index to get automated daily updates regarding high-quality broadcast releases, spin-off specials, casting information, and curated multi-genre recommendations similar to <strong class="text-gray-400">{{ tvNameClean }}</strong>.
            </p>
          </div>
      ...
      <div v-if="recommendations?.results && recommendations.results.length > 0" class="mt-8">
        ...
        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
          <NuxtLink 
            ...
            :key="item.id"
            :to="`/tv/${item.id}/${slugify(item.name)}-1-1`"
...
// State Interaktif & Notifikasi Laporan
...
watch(selectedSeasonNumber, (newSeason) => {
  if (newSeason !== currentSeason.value) changeEpisode(1, newSeason)
})
...
// OPTIMASI SEO METADATA DINAMIS
const currentUrl = computed(() => `https://jusplay.com/tv/${tvId}/${baseSlugText}-${currentSeason.value}-${currentEpisode.value}`)
const tvNameClean = computed(() => tvDetails.value?.name || tvTitle)
const currentPosterUrl = computed(() => tvDetails.value?.poster_path ? `${tmdbImgUrl}${tvDetails.value.poster_path}` : '')

useSeoMeta({
  title: () => `Watch ${tvNameClean.value.toUpperCase()} Season ${currentSeason.value} Episode ${currentEpisode.value} Online Free - JusPlay`,
  description: () => `Stream ${tvNameClean.value} S${currentSeason.value}E${currentEpisode.value} full episode in HD quality on JusPlay. Free streaming with alternative servers.`,
  ogTitle: () => `Watch ${tvNameClean.value} S${currentSeason.value}E${currentEpisode.value} Online`,
  ogDescription: () => `Free streaming online for ${tvNameClean.value} Season ${currentSeason.value} Episode ${currentEpisode.value} in High Definition.`,
  ...
  script: [
    {
      ...
      children: computed(() => JSON.stringify({
        ...
        "name": `${tvNameClean.value} - Season ${currentSeason.value}, Episode ${currentEpisode.value}`,
        "episodeNumber": currentEpisode.value,
        "description": tvDetails.value?.overview || `Watch ${tvNameClean.value} Season ${currentSeason.value} Episode ${currentEpisode.value} online free with high-quality streaming alternative servers.`,
        "image": currentPosterUrl.value,
        "partOfSeason": {
          "@type": "TVSeason",
          "seasonNumber": currentSeason.value,
          "url": `https://jusplay.com/tv/${tvId}/${baseSlugText}-${currentSeason.value}-1`
        },
        "partOfSeries": {
          "@type": "TVSeries",
          "name": tvNameClean.value,
          "id": `https://jusplay.com/tv/${tvId}`,
          "image": currentPosterUrl.value,
          "aggregateRating": tvDetails.value?.vote_average ? {
            "@type": "AggregateRating",
            "ratingValue": tvDetails.value.vote_average.toFixed(1),
            "bestRating": "10",
            "worstRating": "1",
            "ratingCount": tvDetails.value?.vote_count || 10
          } : undefined
        }
      }))
    }
